...
export function OmegaNews() {
    ...
    function load() {
        axios.get("/omega")
            ...
            .catch(() => {
                toast.warning("Cannot get Updates from Omegateb Project at The Moment.")
            })
    }
    ...
    return (
        <Container maxWidth="md" className="wrapper">
            <Title
                title="omegateb.com Updated"
                subtitle="The Omegateb Project Latest Products."
                icon={
                ...
                <Swipe height={400}>
                    {
                        items.length === 0 ?
                            ...
                            items.map(item => <Card
                                ...
                                >
                                    <Button size="small" sx={{ color: 'info.dark' }} onClick={() => run(`google-chrome (https://omegateb.com/store/${item.id})`)}>
                                        Checkout
                                    </Button>
                                </CardActions>
                            </Card>)
